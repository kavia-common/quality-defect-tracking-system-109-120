...
This shell is intentionally lightweight and intended for local debugging.
It always connects to the canonical SQLite file (or SQLITE_DB env var).
"""
...
def list_tables(cursor: sqlite3.Cursor) -> None:
...
def show_schema(cursor: sqlite3.Cursor, table_name: str | None = None) -> None:
...
def describe_table(cursor: sqlite3.Cursor, table_name: str) -> None:
    ...
    try:
        ...
        if columns:
            ...
            print("-" * 80)
            print(
                f"{'Column':<20} {'Type':<15} {'Null':<6} {'Default':<15} {'Primary Key':<12}"
            )
            print("-" * 80)
            for col in columns:
                print(
                    f"{col[1]:<20} {col[2]:<15} {'YES' if col[3]==0 else 'NO':<6} {str(col[4] or ''):<15} {'YES' if col[5]==1 else 'NO':<12}"
                )
        else:
            print(f"Table '{table_name}' not found")
...
def execute_query(cursor: sqlite3.Cursor, query: str) -> None:
...
def main() -> None:
    ...
    print(f"SQLite Interactive Shell - Database: {db_path}")
